# Multi-Source Intelligence Agent — Script Template

Complete, production-ready template for a daily intelligence agent that collects from GitHub, arXiv, RSS, and Twitter.

## Usage

```bash
python3 intelligence_agent.py              # formatted report to stdout
python3 intelligence_agent.py --json       # raw JSON to stdout
python3 intelligence_agent.py --date 2026-07-01  # override date
```
...
Usage:
    python3 intelligence_agent.py [--date YYYY-MM-DD] [--json]

Output: JSON report to stdout (--json) or formatted report to stdout
"""
import os
import sys
import json
...
def fetch_xml(url, retries=3, timeout=20):
...
def collect_github_trending():
    """Collect top trending AI/Agent repos from last 24h by stars."""
    ...
    try:
        # Use topic:ai+topic:agent for quality results (avoid generic repos)
        data = fetch_json(
...
def build_report(results):
    """Build formatted report from collected data (Feishu Markdown)."""
    ...
    lines.append("📰 **RSS 资讯**")
    if "error" in rss:
        lines.append("⚠️ {}".format(rss["error"]))
    else:
...
- **Add sources**: Write a new `collect_*()` function and add to `main()`
- **Change categories**: Modify `search_query` in `collect_arxiv_papers()` — see `references/search-query-patterns.md` for recipes
- **Change GitHub scope**: Adjust `topic:ai+topic:agent+stars:>100` in `collect_github_trending()` for different repo categories
- **Change schedule**: Adjust `schedule='0 8 * * *'` in cron (UTC-based)
- **Change delivery**: Set `deliver` to target channel (origin, telegram, discord, etc.)
- **Add RSS feeds**: Use `blogwatcher-cli add "Name" <url> --feed-url <feed_url>` then `blogwatcher-cli scan` to verify
